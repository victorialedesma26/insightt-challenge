...
const serviceWithRepo = (repo) => new task_service_1.TaskService(repo);
describe('TaskService', () => {
    it('rejects invalid status transitions', async () => {
        const repository = createRepositoryMock();
        repository.findOwnedTask.mockResolvedValue(buildTask());
        const service = serviceWithRepo(repository);
        await expect(service.transitionStatus(TASK_ID, OWNER_ID, { targetStatus: 'DONE' })).rejects.toMatchObject({ code: errorCodes_1.ErrorCodes.INVALID_STATUS_TRANSITION });
    });
    describe('markAsDone', () => {
        it('completes task with atomic update and owner scoping', async () => {
            ...
            expect(repository.updateStatusAtomically).toHaveBeenCalledWith(task.id, OWNER_ID, 'IN_PROGRESS', 'DONE');
        ...
        it('validates owner access', async () => {
            ...
            await expect(service.markAsDone(TASK_ID, OWNER_ID)).rejects.toMatchObject({
                code: errorCodes_1.ErrorCodes.TASK_NOT_FOUND,
            });
        });
        it('rejects when status is not in progress', async () => {
            ...
            const service = serviceWithRepo(repository);
            await expect(service.markAsDone(TASK_ID, OWNER_ID)).rejects.toMatchObject({
                code: errorCodes_1.ErrorCodes.INVALID_STATUS_TRANSITION,
            });
        });
        it('surfaces conflict when atomic update fails and task not done', async () => {
            ...
            const service = serviceWithRepo(repository);
            await expect(service.markAsDone(TASK_ID, OWNER_ID)).rejects.toMatchObject({
                code: errorCodes_1.ErrorCodes.STATUS_CONFLICT,
            });
        });
    });
});
